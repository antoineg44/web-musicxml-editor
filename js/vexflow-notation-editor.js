...
editor.renderer = new Vex.Flow.Renderer(editor.canvas, Vex.Flow.Renderer.Backends.CANVAS);
editor.ctx = editor.renderer.getContext();

editor.chord = [];

editor.canvas = canvas = document.getElementsByTagName('canvas')[0];
		...
		for(i=0; i<editor.staves.length; i++){
			...
			if(editor.staves[i].hasOwnProperty('v1')){
				...
				var voice1 = new Vex.Flow.Voice({
			      ...
			      beat_value: editor.staves[i].timeSigTop,
...
function getMousePos(evt){
    editor.mousePos = editor.select.getMousePos(canvas, evt);
}
